...
frame (`toxin`), and the subsequent arguments are the columns to keep.
...
`filter`, to keep rows where `rain` is greater than 1 cm/week, and then through `select` to
keep the `rain` and `toxin` columns. When the data frame is being passed to the
...
Using pipes, subset the data to include lots from `La Junta`,
and retain the columns `lot`, `town`, and `toxin`.
...
The sixth row has an NA for noon_temp, so if we wanted to remove all observations with a missing value for noon_temp we could insert a `filter()` in this chain:
...
- contains only the `lot` column and a column that contains
values that are the square-root of `wind_speed` values (e.g. a new column
`wind_sqrt`).
- In this `wind_sqrt` column, there are no NA values
and all values are < 3.5.

Hint: think about how the commands should be ordered

```{r, echo=FALSE, eval=FALSE}
challenge <- toxin %>%
  mutate(wind_sqrt = sqrt(wind_speed)) %>%
  filter(!is.na(wind_sqrt)) %>%
  filter(wind_sqrt < 3.5) %>%
  select(lot, wind_sqrt)
...
operations can be run. For example, if we wanted to group by town and find the
number of rows of data for each town, we would do:
...
group into a single-row summary of that group. So to view mean `toxin` by town:
...
Weird warning message: `summarise()` ungrouping output (override with `.groups` argument)

Solution: https://rstats-tips.net/2020/07/31/get-rid-of-info-of-dplyr-when-grouping-summarise-regrouping-output-by-species-override-with-groups-argument/
...
Use `group_by()` and `summarize()` to find the mean, min, and max noon_temp
length for each town
...
There are two lots with `noon_temp` less than 25 degrees Celsius. Let's remove lots an average noon temperature less than 25 degrees.
